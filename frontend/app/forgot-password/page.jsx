...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        setFormError(readError(data) || "Could not reach the server. Please try again.");
        return;
      }
        ...
        <div className="mt-8 rounded-xl border border-line bg-surface p-6 shadow-card">
          ...
          {isSent ? (
            <p className="mt-3 text-sm text-muted-foreground">
              If an account exists for <span className="font-mono text-foreground">{email.trim()}</span>,
            ...
            <>
              ...
              <form onSubmit={handleSubmit} noValidate className="mt-6 space-y-5">
                ...
                <div>
                  ...
                  />
                  {fieldError && (
                    <p className="mt-1.5 font-mono text-[11px] text-destructive">{fieldError}</p>
                  )}
                </div>
